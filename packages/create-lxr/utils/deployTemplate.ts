...
import { readdirSync, writeFileSync, copyFileSync, statSync, mkdirSync } from 'node:fs'
...
const copy = (src: string, dest: string): void => {
  ...
  if (_stat.isDirectory()) copyDir(src, dest)
...
export const deployTemplate = (params: IDeployTemplateParams): void => {
  ...
  const write = (file: string, content?: string): void => {
    ...
    if (content !== undefined) writeFileSync(pathToFileURL(targetPath), content)
